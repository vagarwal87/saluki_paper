...
code=as.factor(unlist(lapply(strsplit(colnames(c[,2:ncol(c)]),"\\_"),function(x) x[1])))
...
type.color.map = c("black","purple","blue","darkorange","darkred") #sample(brewer.pal(nlevels(type), name = "Dark2")) #rainbow(nlevels(type),start=.1,end=.7)
...
nrow(c)
pdf("Fig1_S1.pdf",height=20,width=20)
