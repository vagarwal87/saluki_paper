...
results = data.frame(tests = c("B vs B5","B vs BO","B vs B3","B vs BC","BC vs BCO",
...
pdf("png/violinplots.pdf",width=12,height=4) #Fig3a.pdf
...
topcoefs = unlist(lapply( c(names), function(sample) {
    ...
    cols = c("darkorange", "red", "purple", "cyan","blue")[type]

    pdf(paste("png/", sample,"_CV-Lasso-coefficients.pdf",sep=''),width=8,height=8) #Fig3c.pdf
...
pdf("png/BC3MS.Lasso.pdf") #Fig3b.pdf
...
heatmaps=function(x) {
  ...
  colsrow = c("darkorange", "red", "purple", "cyan","blue")[typerow]
  colscol = c("darkorange", "red", "purple", "cyan","blue")[typecol]
...
pdf("png/Top30Coefs_80Pearson.pdf", width=18,height=7) #Fig3d.pdf
...
a=read.csv("experimentResults_crossSpecies.csv")
round(a,3)

results = data.frame(tests = c("HH vs MH","MM vs HM"), pvals =
c(t.test(a[,1], a[,2], paired=T)$p.value,
t.test(a[,3], a[,4], paired=T)$p.value)
)
results

meltData <- reshape::melt(a)

pdf("png/violinplots_crossSpecies.pdf",width=12,height=4) #FigS4e.pdf
p<-ggplot(meltData, aes(x=variable, y=value)) + geom_violin(position=position_dodge(1))
print(p)
dev.off()
